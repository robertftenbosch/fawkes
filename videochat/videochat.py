...
from rtc_main2 import TARGET_ID
from utils.constants import SIGNALING_SERVER, CLIENT_ID
...
class VideoChatApp(QWidget):
    ...
    async def connect_signaling(self):
            ...
            async for message in self.ws:
                ...
                if data["type"] == "offer":
                    if self.peer_connection.signalingState == "have-local-offer":
                        print("Ignoring duplicate offer.")
                        return

                    print("Received SDP offer, setting remote description...")
                    ...
                    print("Received SDP answer, setting remote description...")
                    if self.peer_connection.signalingState != "have-local-offer":
                        print("Warning: Answer received, but no local offer was made.")
                    await self.peer_connection.setRemoteDescription(
        ...
        # ✅ Only send an offer if we haven't already sent one
        if self.peer_connection.signalingState == "stable":
            ...
            print("Skipping offer creation since signaling state is", self.peer_connection.signalingState)

    def toggle_mute(self):
        self.is_muted = not self.is_muted
        print("Local video track added:", self.video_track)
        for sender in self.peer_connection.getSenders():
            print("Sender track:", sender.track.kind)
